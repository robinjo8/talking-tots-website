...
export const useMemoryGameČ = () => {
  ...
  const { data: memoryCardsData, isLoading, error } = useQuery({
    queryKey: ["memoryCardsČ"],
    queryFn: async () => {
      console.log("Fetching memory cards Č from Supabase...");
  ...
  useEffect(() => {
    if (memoryCardsData && memoryCardsData.length > 0) {
      console.log("Initializing game Č with memory cards data:", memoryCardsData);
