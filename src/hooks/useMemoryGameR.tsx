...
export const useMemoryGameR = () => {
  ...
  const { data: memoryCardsData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      console.log("Fetching memory cards R from Supabase...");
  ...
  useEffect(() => {
    if (memoryCardsData && memoryCardsData.length > 0) {
      console.log("Initializing game R with memory cards data:", memoryCardsData);
