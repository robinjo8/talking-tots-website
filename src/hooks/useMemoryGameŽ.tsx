...
export const useMemoryGameŽ = () => {
  ...
  const { data: memoryCardsData, isLoading, error } = useQuery({
    queryKey: ["memoryCardsŽ"],
    queryFn: async () => {
      console.log("Fetching memory cards Ž from Supabase...");
  ...
  useEffect(() => {
    if (memoryCardsData && memoryCardsData.length > 0) {
      console.log("Initializing game Ž with memory cards data:", memoryCardsData);
