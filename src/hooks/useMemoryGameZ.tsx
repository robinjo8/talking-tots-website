...
export const useMemoryGameZ = () => {
  ...
  const { data: memoryCardsData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      console.log("Fetching memory cards Z from Supabase...");
  ...
  useEffect(() => {
    if (memoryCardsData && memoryCardsData.length > 0) {
      console.log("Initializing game Z with memory cards data:", memoryCardsData);
