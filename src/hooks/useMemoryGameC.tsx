...
export const useMemoryGameC = () => {
  ...
  const { data: memoryCardsData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      console.log("Fetching memory cards C from Supabase...");
  ...
  const initializeGame = (cardData: any[]) => {
    ...
    gameCompletedRef.current = false; // Reset completion tracking
  ...
  useEffect(() => {
    if (memoryCardsData && memoryCardsData.length > 0) {
      console.log("Initializing game C with memory cards data:", memoryCardsData);
  ...
  useEffect(() => {
    if (cards.length > 0 && matchedPairs.length === cards.length / 2 && !gameCompletedRef.current) {
      ...
      gameCompletedRef.current = true; // Prevent multiple recordings
      // Record completion in progress system
      recordGameCompletion('memory', 'C');
    }
  }, [matchedPairs, cards]); // Removed recordGameCompletion from dependencies
