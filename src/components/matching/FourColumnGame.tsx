...
export function FourColumnGame({ items, onGameComplete, isLandscape = false }: FourColumnGameProps) {
  ...
  const getWrittenTileClass = (itemId: string) => {
    const writtenBase = isLandscape 
      ? "w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center border-2 rounded-lg cursor-pointer transition-all duration-200 hover:scale-105 overflow-hidden bg-white p-1"
