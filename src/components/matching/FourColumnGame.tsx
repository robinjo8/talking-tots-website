...
export function FourColumnGame({ items, onGameComplete, isLandscape = false }: FourColumnGameProps) {
  ...
  const getWrittenTileClass = (itemId: string) => {
    if (isItemCompleted(itemId)) return `${baseTileClass} border-green-300 bg-green-100 opacity-60`;
    if (isItemSelected(itemId, 'written')) return `${baseTileClass} border-blue-400 bg-blue-100 border-4`;
    return `${baseTileClass} border-gray-200 hover:border-blue-300`;
  };
  ...
  const getFontSizeForTile = (word: string): string => {
    const ratio = tileSize / 160; // base ratio
    if (word.length <= 3) return `${Math.max(Math.floor(24 * ratio), 12)}px`;
    if (word.length <= 5) return `${Math.max(Math.floor(20 * ratio), 11)}px`;
    if (word.length <= 7) return `${Math.max(Math.floor(16 * ratio), 10)}px`;
    return `${Math.max(Math.floor(14 * ratio), 9)}px`;
  };
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <div className={`relative rounded-xl ${isLandscape ? 'w-full max-w-4xl p-1' : 'p-2'}`}>
        <div className="grid grid-cols-4 justify-items-center" style={{ gap: `${gapSize}px` }}>
          {/* Audio Column */}
          ...
          <div className="flex flex-col" style={{ gap: `${gapSize}px` }}>
            {gameState.shuffledWritten.map((itemId, index) => {
              ...
              if (!item) return null;
              return (
                <div key={`written-${itemId}-${index}`} className={getWrittenTileClass(itemId)} style={baseTileStyle} onClick={() => selectWritten(itemId)}>
                  <span className="font-bold text-center" style={{ fontSize: getFontSizeForTile(item.writtenWord) }}>
                    {item.writtenWord}
                  </span>
                </div>
              );
