...
export const MatchingCompletionDialog: React.FC<MatchingCompletionDialogProps> = ({
  ...
  const startRecording = async (imageIndex: number, word: string) => {
    ...
    try {
      ...
      // Start recording
      recorder.start();
      setRecordingStates(prev => ({ ...prev, [imageIndex]: true }));
      setRecordingTimeLeft(3);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-dragon-green justify-center">
        ...
        <div className="flex justify-center">
          <Button 
            onClick={handleClose}
            className="bg-dragon-green hover:bg-dragon-green/90 gap-2"
          >
            <X className="w-4 h-4" />
            Zapri
          </Button>
