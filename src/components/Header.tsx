        ...
        <div className="flex items-center gap-3">
          ...
          {isMobile ? <Sheet>
              ...
              {user && <>
                  ...
                  {navigationLinks.map((link, index) => <Button key={index} variant="ghost" className={`font-medium ${link.disabled ? 'opacity-50 cursor-not-allowed' : isActivePath(link.path) ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => !link.disabled && handleNavigate(link.path)} disabled={link.disabled}>
