...
interface SlidingPuzzle78Props {
  imageUrl: string;
  onComplete: () => void;
  className?: string;
}
...
export const SlidingPuzzle78: React.FC<SlidingPuzzle78Props> = ({
  imageUrl,
  onComplete,
  className = ""
}) => {
  ...
  // Initialize puzzle pieces for 3x4 grid (12 pieces total, 11 + 1 empty)
  const initializePuzzle = useCallback(() => {
    // Create pieces (0-11, where 11 is empty)
    const initialPieces: PuzzlePiece[] = [];
    for (let i = 0; i < 12; i++) {
      initialPieces.push({
        id: i,
        position: i,
        isEmpty: i === 11 // Last piece is empty
      });
    }
  ...
  // Get valid moves for empty space in 3x4 grid
  const getValidMoves = (emptyIndex: number): number[] => {
    ...
    // Can move down
    if (row < 3) moves.push(emptyIndex + 3); // 4 rows (0, 1, 2, 3)
    // Can move left
    if (col > 0) moves.push(emptyIndex - 1);
    // Can move right
    if (col < 2) moves.push(emptyIndex + 1); // 3 columns (0, 1, 2)
  ...
  const checkCompletion = useCallback((currentPieces: PuzzlePiece[]) => {
    return currentPieces.every((piece, index) => {
      if (piece.isEmpty) return index === 11; // Empty should be at position 11
  ...
  return (
    <div className={`${className} flex justify-center items-center`}>
      <div className="relative">
        ...
        <div className="relative grid grid-cols-3 grid-rows-4 gap-1 bg-gray-800 p-2 rounded-lg shadow-lg" style={{ zIndex: 1 }}>
          {pieces.map((piece, index) => (
            <div
              key={`${piece.id}-${index}`}
              className={`
                w-24 h-24 md:w-48 md:h-48 relative cursor-pointer
              ...
              style={{
                ...
                backgroundSize: '300% 400%', // 3 columns, 4 rows
