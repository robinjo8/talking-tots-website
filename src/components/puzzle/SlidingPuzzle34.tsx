...
interface SlidingPuzzle34Props {
  imageUrl: string;
  onComplete: () => void;
  className?: string;
}
...
export const SlidingPuzzle34: React.FC<SlidingPuzzle34Props> = ({
  imageUrl,
  onComplete,
  className = ""
}) => {
  ...
  // Initialize puzzle pieces for 3x2 grid (6 pieces total, 5 + 1 empty)
  const initializePuzzle = useCallback(() => {
    // Create pieces (0-5, where 5 is empty)
    const initialPieces: PuzzlePiece[] = [];
    for (let i = 0; i < 6; i++) {
      initialPieces.push({
        id: i,
        position: i,
        isEmpty: i === 5 // Last piece is empty
      });
    }

    // Shuffle the puzzle (but keep it solvable)
    const shuffledPieces = [...initialPieces];
    for (let i = 0; i < 50; i++) { // Less shuffling for easier difficulty
  ...
  // Get valid moves for empty space in 3x2 grid
  const getValidMoves = (emptyIndex: number): number[] => {
    ...
    // Can move down
    if (row < 1) moves.push(emptyIndex + 3); // Only 2 rows (0 and 1)
    // Can move left
  ...
  const checkCompletion = useCallback((currentPieces: PuzzlePiece[]) => {
    return currentPieces.every((piece, index) => {
      if (piece.isEmpty) return index === 5; // Empty should be at position 5
