...
interface SlidingPuzzle910Props {
  imageUrl: string;
  onComplete: () => void;
  className?: string;
}
...
export const SlidingPuzzle910: React.FC<SlidingPuzzle910Props> = ({
  imageUrl,
  onComplete,
  className = ""
}) => {
  ...
  // Initialize puzzle pieces for 4x4 grid (16 pieces total, 15 + 1 empty)
  const initializePuzzle = useCallback(() => {
    // Create pieces (0-15, where 15 is empty)
    const initialPieces: PuzzlePiece[] = [];
    for (let i = 0; i < 16; i++) {
      initialPieces.push({
        id: i,
        position: i,
        isEmpty: i === 15 // Last piece is empty
      });
    }
  ...
  // Get valid moves for empty space in 4x4 grid
  const getValidMoves = (emptyIndex: number): number[] => {
    ...
    // Can move down
    if (row < 3) moves.push(emptyIndex + 4); // 4 rows (0, 1, 2, 3)
    // Can move left
    if (col > 0) moves.push(emptyIndex - 1);
    // Can move right
    if (col < 3) moves.push(emptyIndex + 1); // 4 columns (0, 1, 2, 3)
  ...
  const checkCompletion = useCallback((currentPieces: PuzzlePiece[]) => {
    return currentPieces.every((piece, index) => {
      if (piece.isEmpty) return index === 15; // Empty should be at position 15
  ...
  return (
    <div className={`${className} flex justify-center items-center`}>
      <div className="relative">
        ...
        <div className="relative grid grid-cols-4 grid-rows-4 gap-1 bg-gray-800 p-2 rounded-lg shadow-lg" style={{ zIndex: 1 }}>
          {pieces.map((piece, index) => (
            <div
              key={`${piece.id}-${index}`}
              className={`
                w-20 h-20 md:w-40 md:h-40 relative cursor-pointer
              ...
              style={{
                ...
                backgroundPosition: piece.isEmpty ? 'center' : 
                  `${(piece.id % 4) * -100}% ${Math.floor(piece.id / 4) * -100}%`
              }}
            >
