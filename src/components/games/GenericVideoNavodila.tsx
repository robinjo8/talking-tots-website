...
export function GenericVideoNavodila({ title, videoUrl, displayLetter, backPath = "/video-navodila" }: GenericVideoNavodilaProps) {
  ...
  return (
    <div className={`fixed inset-0 flex flex-col overflow-hidden ${isMobile ? 'bg-white' : 'bg-background'}`}>
      ...
      >
        ...
        <div className={`relative w-full ${isMobile ? 'h-full' : 'flex items-center justify-center'}`} style={!isMobile && maxVideoHeight > 0 ? { maxHeight: maxVideoHeight } : undefined}>
          ...
          >
            ...
            >
              <div className={`bg-black/40 backdrop-blur-sm px-2 py-1.5 md:px-6 md:py-3 ${isMobile ? '' : 'rounded-b-lg'}`}>
                <VideoProgressBar
