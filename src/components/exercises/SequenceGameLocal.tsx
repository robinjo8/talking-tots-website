...
interface SequenceGameLocalProps {
  onGameComplete: (images: any[]) => void;
  isLandscape?: boolean;
  localData: { word: string; image_url: string; audio_url: string }[];
}

export const SequenceGameLocal = ({ onGameComplete, isLandscape = false, localData }: SequenceGameLocalProps) => {
  const { targetSequence, currentSequence, isComplete, isLoading, moveItem, correctIndices, isPlayingAudio, activePlayingIndex } = useSequenceGame("local", 4, localData);
  ...
  useEffect(() => {
    if (!isComplete) setGameCompletedTriggered(false);
  }, [isComplete]);

  useEffect(() => {
    if (gamePhase !== "pre-countdown" || targetSequence.length === 0) return;
    if (preCountdown <= 0) { setGamePhase("playing"); return; }
  ...
  const itemSize = useMemo(() => {
    if (!isLandscape || windowSize.width === 0 || windowSize.height === 0) return undefined;
    const columns = 4, rows = 2, gap = 8, PADDING = 8, TEXT_HEIGHT = 40;
  ...
  const handleDrop = (dropIndex: number) => {
    if (draggedIndex !== null && draggedIndex !== dropIndex) moveItem(draggedIndex, dropIndex);
    setDraggedIndex(null);
  };

  if (isLoading) return <div className="flex justify-center items-center min-h-[400px]"><Loader2 className="h-12 w-12 animate-spin text-white" /></div>;
  if (targetSequence.length === 0) return <div className="text-center py-8"><p className="text-white/80 drop-shadow uppercase">NI NA VOLJO SLIK ZA TO IGRO.</p></div>;
  if (isLandscape && !itemSize) return null;

  if (gamePhase === "pre-countdown") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <h3 className="text-xl md:text-3xl font-bold text-white drop-shadow-lg uppercase text-center">PRIPRAVI SE NA IGRO ZAPOREDJA!</h3>
        <div className="text-6xl md:text-8xl font-bold text-red-500 drop-shadow-lg animate-pulse">{preCountdown}</div>
      </div>
    );
  }

  return (
    <div className={`w-full mx-auto ${isLandscape ? 'h-full flex flex-col items-center justify-center' : 'max-w-4xl space-y-2 md:space-y-8 px-2 md:px-0'}`} style={{ touchAction: 'none' }}>
      <div className="relative">
        ...
        <div className={`bg-white/20 backdrop-blur-sm rounded-xl border-2 border-gray-400/50 ${isLandscape ? 'flex justify-center gap-2 p-2 mt-1' : 'grid grid-cols-4 gap-1.5 md:gap-4 p-2 md:p-6 mt-1 md:mt-2'}`} style={isLandscape && itemSize ? { width: gridWidth } : {}}>
          {targetSequence.map((image, index) => (
            <SequenceItem key={`target-${image.id}`} image={image} index={index} isDraggable={false} isTarget={true} size={itemSize} />
          ))}
        </div>
      </div>
      ...
      {!isComplete && (
        <div className={`text-center ${isLandscape ? 'py-1' : 'py-1 md:py-0'}`}>
          <p className={`font-bold text-white drop-shadow-lg uppercase ${isLandscape ? 'text-sm' : 'text-xs md:text-base'}`}>POVLECI IN SPUSTI SLIKE, DA JIH RAZVRSTIŠ</p>
        </div>
      )}

      <div className="relative">
        <div className={`bg-white/30 backdrop-blur-sm rounded-xl border-3 border-orange-400 animate-[pulse-border_2s_ease-in-out_infinite] ${isLandscape ? 'flex justify-center gap-2 p-2' : 'grid grid-cols-4 gap-1.5 md:gap-4 p-2 md:p-6'}`} style={isLandscape && itemSize ? { width: gridWidth } : {}}>
          {currentSequence.map((image, index) => (
            <SequenceItem key={`current-${image.id}-${index}`} image={image} index={index} isDraggable={!isComplete && !isPlayingAudio} isTarget={false} isCorrect={correctIndices.includes(index)} isActivePlaying={activePlayingIndex === index} size={itemSize} onDragStart={handleDragStart} onDrop={handleDrop} />
          ))}
        </div>
        {!isComplete && (
