...
interface SequenceGame56StorageBaseProps {
...
export const SequenceGame56StorageBase = ({ onGameComplete, isLandscape = false, letter }: SequenceGame56StorageBaseProps) => {
  const imageCount = 4;
  const extraImageCount = 4;
  const countdownSeconds = 10;
  const helpDuration = 5;
  const helpAttemptsInitial = 3;
  ...
  const [placedImages, setPlacedImages] = useState<(SequenceStorageImage | null)[]>([null, null, null, null]);
  ...
  const [helpUsesLeft, setHelpUsesLeft] = useState(helpAttemptsInitial);
