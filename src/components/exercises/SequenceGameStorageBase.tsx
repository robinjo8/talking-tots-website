...
export interface SequenceGameStorageConfig {
  imageCount: number;       // Number of images to guess (3-4 for 56, 5 for 78)
  countdownSeconds: number; // Memorize phase duration
  helpAttempts: number;     // Number of help uses
  helpDuration: number;     // How long help shows (seconds)
}

interface SequenceGameStorageBaseProps {
  onGameComplete: (images: any[]) => void;
  isLandscape?: boolean;
  letter: string;
  config: SequenceGameStorageConfig;
}

type GamePhase = "memorize" | "select" | "arrange";
type SlotStatus = "empty" | "correct" | "wrong-position" | "wrong";

export const SequenceGameStorageBase = ({ 
  onGameComplete, 
  isLandscape = false, 
  letter,
  config 
}: SequenceGameStorageBaseProps) => {
  const { imageCount, countdownSeconds, helpAttempts, helpDuration } = config;
  const extraImageCount = Math.min(imageCount, 5);

  const { allImages, isLoading } = useSequenceGameStorage(letter, imageCount + extraImageCount);

  const [targetSequence, setTargetSequence] = useState<SequenceStorageImage[]>([]);
  const [selectionImages, setSelectionImages] = useState<SequenceStorageImage[]>([]);
  const [gamePhase, setGamePhase] = useState<GamePhase>("memorize");
  const [countdown, setCountdown] = useState(countdownSeconds);
  const [placedImages, setPlacedImages] = useState<(SequenceStorageImage | null)[]>([]);
  ...
  const getSlotStatus = useCallback((image: SequenceStorageImage | null, index: number): SlotStatus => {
  ...
  const handleImageSelect = useCallback((image: SequenceStorageImage) => {
  ...
  return (
    ...
    >
      ...
      <SequenceImageSelectionDialog
        ...
        images={selectionImages as any}
        onSelect={handleImageSelect as any}
