...
export const SequenceGameS = ({ onGameComplete, isLandscape = false }: SequenceGameSProps) => {
  const { targetSequence, currentSequence, isComplete, isLoading, moveItem, correctIndices, isPlayingAudio, activePlayingIndex } = useSequenceGame("memory_cards_S", 4);
  ...
  const itemSize = useMemo(() => {
    ...
    // Desktop: account for title, text, padding, buttons
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div 
          className={`bg-white/20 backdrop-blur-sm rounded-xl border-2 border-gray-400/50 flex justify-center gap-2 p-2 mt-1`}
      ...
      <div className="relative">
        <div 
          className={`bg-white/30 backdrop-blur-sm rounded-xl border-3 border-orange-400 animate-[pulse-border_2s_ease-in-out_infinite] flex justify-center gap-2 p-2`}
          style={itemSize ? { width: gridWidth } : {}}
        >
