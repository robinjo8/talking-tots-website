...
export const FeaturesCarousel = () => {
  ...
  return <div className="w-full max-w-6xl mx-auto px-4">
      ...
      <div className="relative overflow-visible">
        <Carousel setApi={setApi} className="w-full overflow-visible" opts={{
        align: "start",
        loop: true,
        containScroll: "trimSnaps"
      }}>
          <CarouselContent className="-ml-2 md:-ml-4">
            {features.map((feature, index) => <CarouselItem key={index} className="pl-2 md:pl-4 sm:basis-1/2 md:basis-1/3 lg:basis-1/4">
                <div className="h-full">
                  <div className="bg-white rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 p-8 h-full flex flex-col border border-gray-100/50 min-h-[300px]">
                    ...
                    <div className="text-center flex-grow flex flex-col justify-between">
                      <div>
                        <h3 className="text-xl font-bold mb-4 leading-tight text-gray-900 line-clamp-2 py-[5px]">
                          {feature.title}
                        </h3>
          ...
          <button onClick={handlePrevClick} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 h-12 w-12 bg-white shadow-lg hover:shadow-xl hover:bg-gray-50 border border-gray-200/50 rounded-full flex items-center justify-center z-10 transition-all duration-300 hover:scale-105" aria-label="Prejšnja funkcija">
            <ChevronLeft className="h-5 w-5 text-gray-700" />
          </button>
          <button onClick={handleNextClick} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 h-12 w-12 bg-white shadow-lg hover:shadow-xl hover:bg-gray-50 border border-gray-200/50 rounded-full flex items-center justify-center z-10 transition-all duration-300 hover:scale-105" aria-label="Naslednja funkcija">
      ...
      <div className="flex justify-center gap-2 mt-8">
        {Array.from({
        length: count
      }).map((_, i) => <button key={i} className={cn("w-2.5 h-2.5 rounded-full transition-all duration-300", i === current ? "bg-dragon-green scale-125 shadow-sm" : "bg-gray-300 hover:bg-gray-400")} onClick={() => {
        api?.scrollTo(i);
        handleManualNavigation();
