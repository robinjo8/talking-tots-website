...
export function UserProfile() {
  ...
  const handleGoToProfile = (options?: {
  ...
  return <div className="flex items-center gap-4">
      <Popover>
        <PopoverTrigger asChild>
          <div className="flex items-center gap-2 px-6 py-3 bg-white border border-gray-200 rounded-full cursor-pointer hover:bg-gray-50 transition-all duration-200 shadow-sm h-12 lg:h-14">
            {selectedChild && selectedChild.avatarId > 0 ? <Avatar className="h-8 w-8 lg:h-10 lg:w-10 border border-green-200">
                <AvatarImage src={getAvatarSrc(selectedChild.avatarId)} alt={selectedChild.name} className="object-contain" />
                <AvatarFallback className="bg-green-100 text-green-800">
                  {selectedChild.name[0]}
                </AvatarFallback>
              </Avatar> : <Avatar className="h-8 w-8 lg:h-10 lg:w-10 border border-gray-200">
        ...
        <PopoverContent className="w-80 p-0 shadow-lg border bg-card text-card-foreground" align="end" sideOffset={4}>
          <div className="p-3 bg-[dragon-green-DEFAULT] bg-emerald-50">
            {/* Parent profile */}
            <div className={`flex items-center gap-2 p-2 rounded-md cursor-pointer ${selectedChildIndex === null ? 'bg-blue-50' : 'hover:bg-gray-100'}`} onClick={() => {
            ...
            localStorage.removeItem('selectedChildIndex');
              ...
              {selectedChildIndex === null && <div className="h-2 w-2 rounded-full bg-blue-600"></div>}
            </div>
            
            {/* Children profiles section */}
            {profile?.children && profile.children.length > 0 && <div className="mt-2">
                ...
                <div className="mt-1 bg-green-50 rounded-md overflow-hidden">
                  {profile.children.map((child, index) => <div key={index} className={`flex items-center gap-2 p-2 cursor-pointer ${selectedChildIndex === index ? 'bg-green-100' : 'hover:bg-green-100/50'}`} onClick={() => handleSelectChild(index)}>
                      <Avatar className="h-8 w-8 border border-green-200">
                        {child.avatarId > 0 ? <AvatarImage src={getAvatarSrc(child.avatarId)} alt={child.name} className="object-contain" /> : <AvatarFallback className="bg-green-100 text-green-800">
                            {child.name[0]}
                          </AvatarFallback>}
                      </Avatar>
                      <span className="text-base flex-1 uppercase">{child.name}</span>
                      {selectedChildIndex === index && <div className="h-2 w-2 rounded-full bg-green-600"></div>}
                    </div>)}
                </div>
              </div>}
          </div>
          ...
          <div className="border-t border-gray-200">
            <div className="p-2 grid grid-cols-1 gap-1 bg-emerald-50">
              {navigationItems.map((item, index) => <div key={index}>
                  <Button variant="ghost" size="sm" className={`w-full justify-start text-left uppercase ${item.disabled ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={() => item.options ? handleGoToProfile(item.options) : navigate(item.path)} disabled={item.disabled}>
                    <item.icon className="h-4 w-4 mr-2" />
                    <span className="text-base">{item.label}</span>
                  </Button>
                </div>)}
              <Button variant="ghost" size="sm" className="w-full justify-start text-left text-red-500 hover:text-red-600 hover:bg-red-50 uppercase" onClick={handleSignOut}>
