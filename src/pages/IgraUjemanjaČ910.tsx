...
export default function IgraUjemanjaČ910() {
  return (
    <AgeGatedRoute requiredAgeGroup="9-10">
      <IgraUjemanjaČ910Content />
    </AgeGatedRoute>
  );
}

function IgraUjemanjaČ910Content() {
  const { user, selectedChild } = useAuth();
  const navigate = useNavigate();
  const letter = 'č'; // Fixed to 'č' for this component
  ...
  const handleStarClaimed = () => {
    recordGameCompletion('matching_č_9-10');
  };
  ...
  // Get letter data for four-column game (fixed to 'č')
  const upperCaseLetter = letter?.toUpperCase() || 'Č';
  
  // Get 4 random items for the four-column game
  const items = getRandomFourColumnItems(4, 'č');
