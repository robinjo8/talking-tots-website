...
import { SequenceGameČ56 } from "@/components/exercises/SequenceGameČ56";
...
export default function ZaporedjaČ56() {
  ...
  const handleStarClaimed = () => { recordGameCompletion('memory', 'sequence_č_5-6'); setShowCompletion(false); setShowNewGameButton(true); };
          ...
          <div className="relative z-10 flex-1 flex items-center justify-center overflow-hidden h-full w-full p-2">
            {!isPortrait ? (
              <div className="w-full h-full flex flex-col items-center justify-center">
                <SequenceGameČ56 key={gameKey} onGameComplete={handleGameComplete} isLandscape={true} />
              </div>
            ) : (
    ...
    <AgeGatedRoute requiredAgeGroup="5-6">
      <div className="fixed inset-0 overflow-auto select-none" style={{ backgroundImage: `url('${backgroundImageUrl}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
        <div className="min-h-full flex flex-col items-center justify-center p-4 pb-24">
          ...
          <SequenceGameČ56 key={gameKey} onGameComplete={handleGameComplete} />
        </div>
        <div className="fixed bottom-4 left-4 z-50 flex items-center gap-3">
          <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
            <DropdownMenuTrigger asChild><button className="w-16 h-16 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 flex items-center justify-center shadow-lg border-2 border-white/50 backdrop-blur-sm hover:scale-105 transition-transform"><Home className="w-8 h-8 text-white" /></button></DropdownMenuTrigger>
