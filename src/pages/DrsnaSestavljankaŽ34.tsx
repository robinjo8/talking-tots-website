...
const žImages = [{ filename: 'zaba.png', word: 'ŽABA' },{ filename: 'zaga.png', word: 'ŽAGA' },{ filename: 'zarnica.png', word: 'ŽARNICA' },{ filename: 'zebelj.png', word: 'ŽEBELJ' },{ filename: 'zelva.png', word: 'ŽELVA' },{ filename: 'zerjav.png', word: 'ŽERJAV' },{ filename: 'zirafa.png', word: 'ŽIRAFA' },{ filename: 'zlica.png', word: 'ŽLICA' },{ filename: 'zoga.png', word: 'ŽOGA' },{ filename: 'zolna.png', word: 'ŽOLNA' }];

export default function DrsnaSestavljankaŽ34() { return (<AgeGatedRoute requiredAgeGroup="3-4"><DrsnaSestavljankaŽ34Content /></AgeGatedRoute>); }
...
function DrsnaSestavljankaŽ34Content() {
  ...
  const completionImages = useMemo(() => { const shuffled = [...žImages].sort(() => Math.random() - 0.5); return shuffled.slice(0, 4).map(img => ({ word: img.word, url: `https://ecmtctwovkheohqwahvt.supabase.co/storage/v1/object/public/slike/${img.filename}`, filename: img.filename })); }, [puzzleKey]);
  ...
  const handleConfirmNewGame = () => { gameCompletedRef.current = false; setPuzzleKey(prev => prev + 1); setShowNewGameDialog(false); setShowNewGameButton(false); };
  const handleStartNewGameDirect = () => { gameCompletedRef.current = false; setPuzzleKey(prev => prev + 1); setShowNewGameButton(false); };
  const handleBack = () => { setMenuOpen(false); setShowExitDialog(true); };
  const handleStarClaimed = () => { recordGameCompletion('sliding_puzzle', 'sliding_puzzle_ž_3-4'); setShowCompletion(false); setShowNewGameButton(true); };
  ...
  return (
    <div className="fixed inset-0 overflow-auto select-none" style={{ backgroundImage: 'url(https://ecmtctwovkheohqwahvt.supabase.co/storage/v1/object/public/ozadja/zeleno_ozadje.png)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
      <div className="min-h-full flex flex-col items-center justify-center p-4 pb-24"><SlidingPuzzle34 key={puzzleKey} imageUrl={imageUrl} onComplete={handleComplete} /></div>
