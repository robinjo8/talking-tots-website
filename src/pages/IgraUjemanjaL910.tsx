...
export default function IgraUjemanjaL910() {
  return (
    <AgeGatedRoute requiredAgeGroup="9-10">
      <IgraUjemanjaL910Content />
    </AgeGatedRoute>
  );
}

function IgraUjemanjaL910Content() {
  const { user, selectedChild } = useAuth();
  const navigate = useNavigate();
  const letter = 'l'; // Fixed to 'l' for this component
  const isMobile = useIsMobile();
  const [gameKey, setGameKey] = useState(0);
  const [showInstructions, setShowInstructions] = useState(false);
  const [showCompletion, setShowCompletion] = useState(false);
  const [items, setItems] = useState<FourColumnMatchingItem[]>(() => getRandomFourColumnItems(4, 'l'));
  ...
  const handleStarClaimed = () => {
    recordGameCompletion('matching_l_9-10');
  };
  ...
  // Get letter data for four-column game (fixed to 'l')
  const upperCaseLetter = letter?.toUpperCase() || 'L';
  ...
  const handleNewGame = () => {
    ...
    const newItems = getRandomFourColumnItems(4, 'l');
    setItems(newItems);
    setCompletedItems([]);
    setMenuOpen(false);
