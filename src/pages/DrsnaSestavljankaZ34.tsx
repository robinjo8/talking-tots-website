...
export default function DrsnaSestavljankaZ34() { return (<AgeGatedRoute requiredAgeGroup="3-4"><DrsnaSestavljankaZ34Content /></AgeGatedRoute>); }
...
function DrsnaSestavljankaZ34Content() {
  ...
  const completionImages = useMemo(() => { const shuffled = [...zImages].sort(() => Math.random() - 0.5); return shuffled.slice(0, 4).map(img => ({ word: img.word, url: `https://ecmtctwovkheohqwahvt.supabase.co/storage/v1/object/public/slike/${img.filename}`, filename: img.filename })); }, [puzzleKey]);
  ...
  const handleStarClaimed = () => { recordGameCompletion('sliding_puzzle', 'sliding_puzzle_z_3-4'); setShowCompletion(false); setShowNewGameButton(true); };
