...
import { SequenceGameK56 } from "@/components/exercises/SequenceGameK56";
...
export default function ZaporedjaK56() {
  ...
  const handleStarClaimed = () => { recordGameCompletion('memory', 'sequence_k_5-6'); setShowCompletion(false); setShowNewGameButton(true); };
          ...
          <div className="relative z-10 flex-1 flex items-center justify-center overflow-hidden h-full w-full p-2">
            {!isPortrait ? (
              <div className="w-full h-full flex flex-col items-center justify-center">
                <SequenceGameK56 key={gameKey} onGameComplete={handleGameComplete} isLandscape={true} />
              </div>
            ) : (
    ...
    <AgeGatedRoute requiredAgeGroup="5-6">
      <div className="fixed inset-0 overflow-auto select-none" style={{ backgroundImage: `url('${backgroundImageUrl}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
        <div className="min-h-full flex flex-col items-center justify-center p-4 pb-24">
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-6 md:mb-8 text-center drop-shadow-lg">ZAPOREDJA - K</h1>
          <SequenceGameK56 key={gameKey} onGameComplete={handleGameComplete} />
        </div>
        <div className="fixed bottom-4 left-4 z-50 flex items-center gap-3">
          <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
