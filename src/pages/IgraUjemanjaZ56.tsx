...
export default function IgraUjemanjaZ56() {
  const { user, selectedChild } = useAuth();
  const navigate = useNavigate();
  const letter = 'z'; // Fixed to 'z' for this component
  ...
  const [items, setItems] = useState<ThreeColumnMatchingItem[]>(() => getRandomThreeColumnItems(4, 'Z'));
  ...
  // Get letter data for three-column game (fixed to 'z')
  const upperCaseLetter = letter?.toUpperCase() || 'Z';
  ...
  const handleNewGame = () => {
    ...
    setCompletedItems([]);
    setItems(getRandomThreeColumnItems(4, 'Z'));
