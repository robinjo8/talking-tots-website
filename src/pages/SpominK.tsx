...
export default function SpominK() {
  ...
  const handleReset = () => {
    ...
    setGameTime(null);
    toast({ title: "Igra je bila ponovno nastavljena!" });
  };
  ...
  useEffect(() => {
    if (gameCompleted && gameStartTimeRef.current && gameTime === null) {
      ...
      setTimeout(() => {
        toast({ title: "Čestitamo!", description: `Igra je končana v ${timeTaken} sekundah!` });
      }, 500);
    }
    ...
    return (
      <div className="fixed inset-0 overflow-hidden select-none">
        ...
        <div className="relative z-10 flex-1 flex items-stretch justify-center overflow-hidden h-full w-full">
          {!isPortrait ? (
            isLoading ? (
              <div className="flex items-center justify-center h-full"><div className="text-lg text-muted-foreground">Nalaganje igre...</div></div>
            ) : error ? (
        ...
        <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
          <DropdownMenuTrigger asChild>
            <Button className="fixed bottom-4 left-4 z-50 rounded-full w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 shadow-lg border-2 border-white/50 backdrop-blur-sm" size="icon">
          ...
          <div className="w-full max-w-4xl mx-auto">
            ...
            {!isLoading && !error && cards.length > 0 && (
              <>
                ...
                <div className="mt-8"><MemoryGrid cards={cards} onCardClick={handleCardClick} isCheckingMatch={isCheckingMatch} /></div>
              </>
            )}
            {!isLoading && !error && cards.length === 0 && (
              <div className="text-center p-10 border rounded-lg"><p className="text-muted-foreground">Ni kartic za prikaz.</p></div>
            )}
          </div>
        </div>
      </div>
