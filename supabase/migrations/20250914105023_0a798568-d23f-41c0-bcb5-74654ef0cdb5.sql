-- Security Enhancement: Protect Children's Personal Data (Fixed)
...
CREATE TABLE IF NOT EXISTS public.children_access_log (
  ...
  access_type TEXT NOT NULL, -- 'view', 'edit', 'delete', 'create'
...
SELECT 
  ...
  CASE WHEN speech_development IS NOT NULL THEN jsonb_typeof(speech_development) ELSE NULL END as has_development_data,
  created_at,
  updated_at
...
-- 11. Create audit triggers for INSERT, UPDATE, DELETE
...
-- 13. RLS Policies for admin permissions
...
-- 14. Create function to safely grant admin roles (only by super admins)
