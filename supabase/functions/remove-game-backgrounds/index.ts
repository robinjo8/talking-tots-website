...
Deno.serve(async (req) => {
  try {
    ...
    const images = [
      { name: 'spomin_2.png' },
      { name: 'sestavljanka_2.png' },
      { name: 'drsna_2.png' },
      { name: 'povezi_pare_1.png' },
    ];
    ...
    for (const img of images) {
      try {
        ...
        // Get original image URL from slike-ostalo bucket
        ...
        // Upload back to slike-ostalo bucket (overwrite original)
        const { error: uploadError } = await supabaseClient.storage
          .from('slike-ostalo')
          .upload(img.name, pngBuffer, {
